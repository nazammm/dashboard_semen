...
- **Halaman Tanya AI dihapus** sepenuhnya (nav, router, file, dan kode `AITools` yang tidak lagi dipakai di `data.js`). RPC terkait di Supabase (`ai_store_mom_change`, dst) dibiarkan ada di database, tidak dihapus — tidak masalah kalau tidak dipakai, tinggal bilang kalau mau dibersihkan juga.
- **Tim Sales**: tombol filter "Hanya Salesman Aktif" dihapus — sekarang otomatis selalu memfilter ke salesman berstatus aktif (`status = 'Sales Real'`), tidak ada lagi opsi untuk menampilkan yang non-aktif.
- **Tim Sales → Detail Transaksi**: klik nama salesman sekarang juga menampilkan dua grafik tren sepanjang tahun (Toko Aktif per Bulan & Tonase per Bulan, aktual vs target), selain daftar transaksinya. Ini nyaris tanpa biaya tambahan karena datanya (`v_salesman_performance`) sudah ter-cache dari halaman Tim Sales — tinggal difilter per salesman di browser, tidak ada query baru ke database.
- **Beranda**: default periode diganti dari YTD ke **MTD** (bulan berjalan).
- **Audit ulang "AI slop"** setelah fitur-fitur baru ditambahkan (Delivery, hero Top Distributor, drill-down salesman, dll). Ditemukan 4 sisa em-dash di komentar kode (`transaksi-detail.js`, `data.js`, `index.html`) — sudah ditulis ulang. Dicek juga tidak ada buzzword generik atau hedging language baru yang ikut masuk.

## Redesain Peta Toko

- **Skala waktu diubah ke MTD** (bulan berjalan), bukan lagi status aktif sepanjang masa. Didukung fungsi database baru `dashboard_toko_monthly(tahun, bulan)` yang menghitung status aktif/nonaktif tiap toko khusus untuk satu bulan tertentu — jumlah baris yang diambil tetap sama (~8.041, satu per toko), cuma angkanya sekarang berbasis bulan berjalan.
- **Semua KPI card sekarang reaktif terhadap filter Distributor** (Total Toko, Aktif Bulan Ini, Nonaktif Bulan Ini, Tanpa Titik Koordinat) — sebelumnya statis dari seluruh data. Dicek langsung ke database: variasinya signifikan antar distributor (PTK 264 dari 930 toko belum punya koordinat, BMM cuma 36 dari 1005), jadi memang penting untuk berubah sesuai filter.
- **Card "Tanpa Titik Koordinat" sekarang bisa diklik** — membuka daftar toko yang belum punya koordinat (nama, kode, distributor, zona, transaksi terakhir), supaya tim bisa tahu persis toko mana yang perlu di-geocode, bukan cuma angka mentah.
- **Pencarian nama/kode toko** ditambahkan sebagai kotak pencarian mengambang di pojok kiri atas peta (bukan form terpisah) — hasil muncul sebagai daftar kecil di bawah kotak, klik untuk lompat ke lokasinya di peta. Toko tanpa koordinat tetap muncul di hasil pencarian (ditandai "tanpa koordinat") supaya tetap bisa dicek meski tidak bisa ditampilkan di peta.
- **Kartu "Distributor Tercakup" dihapus** (jadi kurang informatif begitu ada dropdown filter distributor) diganti posisinya oleh kartu "Tanpa Titik Koordinat" yang lebih actionable.
- **Legend dipindah jadi kartu mengambang** di pojok kiri bawah peta (bukan baris teks di atas peta), dan ditambahkan tombol reset tampilan peta di pojok kanan atas — mengikuti pola UI peta modern (kontrol mengambang di atas peta, bukan memenuhi ruang di sekitarnya) supaya peta terasa lebih lega dan konsisten dengan referensi desain map dashboard saat ini.
